<template lang="html">
	<div class="">
		<div class="add">
			<router-link to='/adminpanel/creature'>back</router-link>
		</div>
		...
		<form class="" method="post">
			<div class="inputblock">
				...
				<label for="">
					atack_speed
					<input type="number" name="atack_speed" v-model="inputs.atack_speed">
				</label>
...
export default {
	...
	async asyncData({params, env, redirect}){
		...
		if(params.id){
			...
			if(params.edit!='edit'){
				return redirect('/adminpanel/creature/')
			}
	...
	methods:{
		...
		buttonclick(){
			...
			var link = '/admin/creature/'+this.edit+'/'+ id;
			console.log(link);
			// return
			$.ajax({
				url:link,
		...
		delete_item(){
			var id=''
			var link = '/admin/creature/delete/'+ this.itemid;
			$.ajax({
				url:link,
				type:'POST',
				dataType:'json',
				success:(data)=> {
					console.log(data);
					if(data.status==200){
						this.$router.push('/adminpanel/creature')
					}
				}
			})
		}
